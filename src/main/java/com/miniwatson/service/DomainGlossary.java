...
public final class DomainGlossary {
    ...
    /**
     * 엣지(관계) 타입. co-occurrence만으론 "같이 나왔다"만 알지 "왜 이어지나"를 모른다.
     * 방향성 있는 관계로 멀티홉 경로 자체가 답의 근거가 되게 한다. UNTYPED는 시드/규칙이
     * 못 잡은 공출현 폴백 — 재현율은 지키면서 점진 전환을 가능케 한다.
     */
    public enum EdgeType {
        SUPERVISES,    // 감독: 기관 -> 기업/시장/상품
        GOVERNED_BY,   // 규제근거: 상품/공시/행위 -> 법령
        ISSUES,        // 발행: 기업 -> 상품
        LISTED_ON,     // 상장/거래: 상품/행위 -> 시장/기관
        CALCULATES,    // 산출: 기관 -> 지수
        REQUIRES,      // 의무: 법령 -> 공시
        CLASSIFIES,    // 분류: 법령 -> 상품유형
        DISCLOSES,     // 공시: 기업 -> 공시
        UNTYPED        // 타입 미상 공출현(폴백)
    }

    /** 시드 관계 1건. 방향성 있음(from -> to). */
    public record Relation(String from, EdgeType type, String to) {}

    /**
     * 선험 관계 시드표 (LLM 0회, 결정적). 두 엔티티가 한 문서에 함께 등장하고 여기 관계가 있으면
     * 그 타입으로 엣지를 단다(없으면 UNTYPED). 자본시장(공시·IR) 도메인의 핵심 관계만 촘촘히.
     */
    public static final java.util.List<Relation> RELATIONS = java.util.List.of(
        // 산출: 한국거래소 -> 지수
        new Relation("한국거래소", EdgeType.CALCULATES, "코스피200"),
        new Relation("한국거래소", EdgeType.CALCULATES, "코스피"),
        new Relation("한국거래소", EdgeType.CALCULATES, "코스닥"),
        // 상장/거래 시장: 행위/지수 -> 한국거래소
        new Relation("공매도", EdgeType.LISTED_ON, "한국거래소"),
        new Relation("기업공개", EdgeType.LISTED_ON, "한국거래소"),
        new Relation("코스피200", EdgeType.LISTED_ON, "한국거래소"),
        // 의무: 자본시장법 -> 공시
        new Relation("자본시장법", EdgeType.REQUIRES, "사업보고서"),
        new Relation("자본시장법", EdgeType.REQUIRES, "분기보고서"),
        new Relation("외부감사법", EdgeType.REQUIRES, "감사보고서"),
        // 분류: 자본시장법 -> 상품유형
        new Relation("자본시장법", EdgeType.CLASSIFIES, "파생결합증권"),
        new Relation("자본시장법", EdgeType.CLASSIFIES, "주가연계증권"),
        new Relation("자본시장법", EdgeType.CLASSIFIES, "상장지수증권"),
        // 규제근거: 상품/행위 -> 법령
        new Relation("주가연계증권", EdgeType.GOVERNED_BY, "자본시장법"),
        new Relation("상장지수펀드", EdgeType.GOVERNED_BY, "자본시장법"),
        new Relation("공매도", EdgeType.GOVERNED_BY, "자본시장법"),
        // 감독: 기관 -> 시장/기관
        new Relation("금융위원회", EdgeType.SUPERVISES, "한국거래소"),
        new Relation("금융감독원", EdgeType.SUPERVISES, "한국거래소"),
        new Relation("금융감독원", EdgeType.SUPERVISES, "사업보고서")
    );

    /**
     * 두 엔티티 사이 시드 관계 타입을 찾는다. 입력 순서 무관(무방향 조회).
     * 방향은 시드표 기준으로 보존하되, 역방향 동시언급도 같은 타입으로 인정한다(Phase A 단순화).
     * 못 찾으면 null → 호출 측에서 UNTYPED로 폴백.
     */
    public static EdgeType relationType(String a, String b) {
        for (Relation r : RELATIONS) {
            if (r.from().equals(a) && r.to().equals(b)) return r.type();
            if (r.from().equals(b) && r.to().equals(a)) return r.type();
        }
        return null;
    }
